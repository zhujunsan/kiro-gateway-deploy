...
def _launch_argv() -> list[str]:
    ...
    # macOS / Windows frozen: launch the bundled executable directly.
    # Using `open -a <bundle>` made the login item show "open" instead of the
    # app name, because launchd resolves the item from ProgramArguments[0].
    # AssociatedBundleIdentifiers only works for signed apps, so it didn't help.
    # The bare executable shows "KiroGatewayTray" (correct name, gray icon).
