...
**Fixed**
- 修复 v0.4.41 升级 cloudflared 2026.8.3 后隧道永远「已停止」：该版本删除了隐藏参数 `--stdin-control`（TUN-10820），进程启动即退出。现钉回仍保留该通道的 2026.8.2，切网软重连继续可用；darwin 官方 checksum 仍有误，已按实际 GitHub 资产 pin。以后若再升到 8.3，关掉开关即可走进程重启，不会再静默起不来。
...
**New**
- 网络切换（WiFi / 有线 / 默认路由变更）时通过 OS 原生路由事件即时感知，经 cloudflared `--stdin-control` 软重连全部边缘连接（免退避、不杀进程）；失败则升级为进程重启，原有 60 秒 `/ready` 超时仍作兜底。
- 隧道状态真实化：解析 `/ready` 的 `readyConnections`，并以观测峰值为期望连接数；新增穿隧道 `GET /health` 端到端探活。菜单显示 `运行中 (4/4 连接)` / `降级 (1/4 连接)` / `降级 (边缘不可达)`，不再仅凭「进程活着 + /ready 200」误报运行中。
...
**Fixed**
- 修复 Windows 托盘菜单打开后「进行中」对话不再动态变化：将 pystray 实例初始化时缓存的 `WM_NOTIFY` 处理器重绑到实时刷新钩子，使菜单打开/关闭事件和每秒原地更新真正生效。
- 移除 Windows 原生菜单为勾选标记/位图预留但未使用的左侧空白栏，并将紧凑样式应用到所有子菜单。
...
**Fixed**
- 修复网关 supervisor 健康探测竞态：启动已失败并写入 error 后，后台探针不再把状态覆写为 starting，避免 Ubuntu CI 打包测试误失败；失败原因继续保留供托盘展示。
...
**Changed**
- 升级内嵌 cloudflared 二进制至 2026.7.3（含 golang.org/x/text CVE 修复）；darwin 资产的官方 release 正文 checksum 仍有误，已按实际 GitHub asset 重新计算并 pin。
...
**Fixed**
- 修复「进行中」子菜单在请求全部结束后叠出一长串“暂无进行中的对话”的问题：仍预留 10 个稳定槽位供菜单展开时原地扩容，但空闲时只显示一条占位文案，其余容量槽隐藏。

## v0.4.9 (2026-07-16)

**Fixed**
- 修复超大请求体被活动面板截断后模型显示为 `unknown`、问题预览为空、输入 token 卡在约 `1M` 的问题：改为头部 + 滚动尾部捕获，并从残缺 JSON 中兜底解析 `model` 与 `<user_query>`；遥测聚合侧同步恢复截断请求的模型名。
...
**Changed**
- 同步上游网关至 `main-bc74347`，docker-compose 镜像与 README 技术说明同步更新。

**Fixed**
- 修复 macOS 托盘菜单卡死后可能劫持系统键盘输入的问题：菜单跟踪期间禁止整体及子菜单结构重建，活动快照改为后台刷新，所有 AppKit 更新统一回到主线程，并在菜单跟踪循环完全退出后才执行延迟重建。
- 最近对话在菜单展开期间仅原地更新现有行，条目减少时安全清理残留内容，新增条目需要扩容时延迟到菜单关闭后处理。

## v0.4.6 (2026-07-16)

**Changed**
- 「进行中」子菜单预留 10 个稳定槽位；空槽统一显示灰色“暂无进行中的对话”，请求结束后立即释放槽位。

**Fixed**
- 修复 Tray 菜单保持展开时只能更新已有行、无法随并发请求动态增加可见项目的问题；macOS、Windows 与 Linux 均可原地更新槽位标题及启用状态，无需重建正在跟踪的菜单。

## v0.4.5 (2026-07-16)

**New**
- 上游模型列表改为按需发现：仅模型列表接口触发，并按账户以 4 小时窗口节流；首次失败回退静态列表，刷新失败保留旧缓存。

**Changed**
- GPT-5.6 Sol / Terra / Luna 与 `auto` 改用真实模型名，Claude 等仍使用 `kiro-*` 别名；托盘模型菜单与使用文档同步更新。
- 配置写入改为锁保护的同目录原子替换；遥测密钥由托盘父进程在启动前持久同步，子进程内用线程安全共享密钥联动 usage 与 incident 上报。
- 同步上游网关至 `main-4905ff6`，docker-compose 镜像 pin 至 `ghcr.io/zhujunsan/kiro-gateway:main-4905ff6`，并开放模型发现缓存窗口配置。

**Fixed**
- 完善无效模型、容量不足、重试退避与流式传输故障分类，避免生成请求额外触发模型发现，并让 OpenAI、Anthropic 与 Responses 返回一致且可操作的错误。
- 修复活动菜单流式 token 统计：跨 ASGI 分片的 SSE、reasoning/thinking、工具名与参数均可计数，最终官方 usage 会覆盖估算；未知输出显示 `—`，真实零值仍显示 `0`。
- 修复活动快照节流写入可能永久丢失或被延迟定时器覆盖，以及 Supervisor 停止后立即重启可能复活旧健康检查线程的问题。
- 遥测上传不再把账户内其他客户端产生的 Credit 消耗归因给零 token 网关桶；历史待重试数据同样在上传前清洗。
...
**New**
- 「进行中」改为多行展示，并显示 ⬆/⬇ tokens（流式过程可动态刷新；有官方 usage 用真实值，否则按字节粗估）。
- 「最近对话」同步显示 ⬆/⬇ tokens。

**Fixed**
- 修复菜单开着时「进行中」仍钉住上一个「已完成」请求、新请求进来不切换的问题。
...
**New**
- Windows / Linux 托盘菜单打开时同步刷新状态与活动列表；弹窗期间以原地改标题方式实时更新「进行中」耗时与网关/隧道状态（避免 `DestroyMenu` / 整表重建打断弹出菜单）。

**Changed**
- macOS 打开菜单时同步刷新网关/隧道连接状态，并重建「最近对话 / 进行中」子菜单；子菜单挂 `menuNeedsUpdate:`，展开即为最新快照。
- 进行中阶段文案改为「等待响应 / 生成中」；对话预览优先提取 `<user_query>` 正文，并剥离 IDE 注入的 open-files / attachment 等噪声块。
- 同步上游网关镜像 / vendor pin 至 `main-9fe2b5e`。

**Fixed**
- 修复菜单已显示「空闲」但子菜单仍残留旧「进行中」行的不同步；弹窗打开期间结构性增删改为关菜单后再整表重建。

## v0.3.20 (2026-07-14)

**Changed**
- 同步上游网关至 `main-a6fd965`：对间歇性 `400 INVALID_MODEL_ID` 做同账号短退避重试（默认额外 2 次，0.5s / 1.0s），减轻「同一模型忽成忽败」；其它 400 仍立即返回。

## v0.3.19 (2026-07-14)

**New**
- 遥测新增平均 TTFT 与输出 token/s：客户端按「首个非空响应 body」记 TTFT，流式请求按「首 token → 结束」生成窗累计 `generation_*`；Worker/D1 落库与日汇总，报告可按总览/用户/天/模型查看。
